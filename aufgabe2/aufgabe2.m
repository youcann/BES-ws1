...
n = zeros(nVoltages, nTimesteps); n(:,1)=n_0*ones(1,nVoltages);
...
m = zeros(nVoltages, nTimesteps); m(:,1)=m_0*ones(1,nVoltages); 
mdot = zeros(nVoltages, nTimesteps); 
alpha_h = zeros(nVoltages, nTimesteps); 
beta_h = zeros(nVoltages, nTimesteps); 
h = zeros(nVoltages, nTimesteps); h(:,1)=h_0*ones(1,nVoltages);
...
for iVoltage=1:nVoltages
    for iTimestep=1:nTimesteps
        ...
        % Kalium
        I_K(iVoltage,iTimestep) = G_K_max * n(iVoltage,iTimestep).^4 * Vm_current;

        % Natrium
        I_Na(iVoltage,iTimestep) = G_Na_max * m(iVoltage,iTimestep).^3 * h(iVoltage,iTimestep) * Vm_current;
        ...
        hdot(iVoltage,iTimestep) = alpha_n(iVoltage,iTimestep) .* (1-h(iVoltage,iTimestep)) - beta_h(iVoltage,iTimestep) .* h(iVoltage,iTimestep);
